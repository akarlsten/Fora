...
const PostItem = ({ owner, content, color }) => {
  return (
    <div className={'flex'}>
      <div className={`px-4 py-2 justify-center items-center flex flex-col self-center w-56 border-r border-${color || 'pink'}-200`}>
        <span className="font-bold">
          {owner.name}
        </span>
        {owner.avatar ? (
          <img className={`my-2 border border-${color || 'pink'}-200`} src={owner.avatar.publicUrlTransformed} alt="" />
        ) : (
      ...
      <div className="px-4 py-2"><ReactMarkdown source={content} /></div>
