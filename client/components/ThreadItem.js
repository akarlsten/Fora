...
const ThreadItem = ({ forumUrl, url, title, count, color, perPage, lastPoster }) => {
  ...
  return (
    ...
    <div className="flex flex-wrap justify-between items-center text-xs xs:text-sm sm:text-base">
      <Link href="/f/[url]/[tid]" as={`/f/${forumUrl}/${url}`}>
        <a className="px-4 py-2 font-semibold flex-grow">{title}</a>
      </Link>
      <div className={'flex justify-end items-center flex-grow'}>
        ...
        <div className={`sm:border-l border-${color || 'pink'}-200 px-2 py-2 text-center w-18 sm:w-32 xl:w-42 font-bold`}>
